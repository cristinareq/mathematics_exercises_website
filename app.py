...
# ------------- CONFIG / SUPABASE -------------
# Expect Streamlit secrets set in .streamlit/secrets.toml:
# SUPABASE_URL = "..."
# SUPABASE_KEY = "..."
...
def format_paris_human(ts_iso: str) -> str:
  """
  Format ISO timestamp string to 'DD/MM/YYYY HHhMM' in Europe/Paris.
  """
  if not ts_iso:
      return "-"
  try:
      # pandas handles tz-aware and naive iso strings well
      dt = pd.to_datetime(ts_iso, utc=True).tz_convert(PARIS_TZ)
      return dt.strftime("%d/%m/%Y %Hh%M")
  except Exception:
      # Fallback: try datetime.fromisoformat (may or may not have tz)
      try:
...
def generate_operand_b(number_type: str, rng: Tuple[int, int]) -> tuple[Union[int, Fraction], str]:
  """
  Returns (value, display_str).
  - Entiers: integer b, display as "b"
  - Négatifs: negative integer b, display as "-b"
  - Fractions: proper or improper fraction with denominator > 1 and not divisible,
    display as "num/den" (unsimplified visual, though Fraction will be simplified internally).
  """
  lo, hi = rng
  ...
  if number_type == "Fractions":
      # choose a denominator >= 2, avoid trivial denominators and divisible numerators
      den = random.randint(2, max(2, hi))
      num = random.randint(lo, hi)
      # avoid reducible/display-ambiguous cases (e.g., 6/2)
      # try a few times for a coprime pair, but don't loop forever
...
def generate_question(cfg: QuizConfig) -> Question:
  # a is always an integer in the selected range
  a = random.randint(cfg.range_min, cfg.range_max)
  # For divide, avoid zero divisors
  if cfg.operation == "Divide":
      if cfg.number_type == "Fractions":
          # keep division simpler: divide by a non-zero integer
  ...
  # Keep the displayed operand for clarity (e.g., "6/2" even if Fraction simplifies)
